class LinksController < ApplicationController
	...
	before_action :validate_attachment_link, only: :donwload_attachment

	
	def download
		@link.update_downloads
		@product = @link.product
		@link.create_attachment_link
		@link_attachments = @link.links
	end

	def download_attachment
		#si no esta el archivo en la carpeta publica se debe especificar la ruta del archivo
	...
	def validate_attachment_link
		if @link_attachment.is_invalid?
			redirect_to invalid_path, notice: "Expiró el tiempo para descargar el archivo"
		end
	end
	...
	def validate_link
		if @link.nil? || @link.is_invalid?
			redirect_to invalid_path, notice: "El link de descarga no es vàlido o ha expirado"
		end
	end
end