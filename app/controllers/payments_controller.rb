class PaymentsController < ApplicationController
	...
	def checkout
		...
		if @my_payment.nil?
			...
			Stores::Paypal.checkout(params[:PayerID], params[:paymentId]) do
				#esta guardando el email de paypal, pero tiene que hacer otra busqueda a la API de paypal
	...
	def create
		...
		#crea el pago pero aun no registra en paypal, es solo para ver si se puede hacer el pago y redirect to paypal
		if paypal_helper.process_payment.create
			...
			#[{href: "dominio.com", method: "algo"},{method: "REDIRECT", href="paypal.com"}]
		else
